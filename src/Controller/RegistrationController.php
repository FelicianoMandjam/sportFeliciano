...
class RegistrationController extends AbstractController
{
     ...
     ): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $this->emailVerifier->sendEmailConfirmation('visitor.registration.verify_email', $user,
                (new TemplatedEmail())
                    ->from(new Address('blogsport@gmail.com', 'Feliciano Mandjam'))
                    ->to($user->getEmail())
                    ->subject('Validation du compte par email sur notre site BLOG FITZONE.')
    ...
    public function waitingForEmail(): Response
    {
    ...
    ): Response
    {
        ...
        $user->isVerified(new \DateTimeImmutable());
        $em->persist($user);
        $em->flush();

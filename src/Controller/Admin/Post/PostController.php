...
class PostController extends AbstractController
{
    ...
    public function publish(Post $post , Request $request, EntityManagerInterface $em): Response 
    {
        ...
        if ($this->isCsrfTokenValid('publish_post'.$post->getId(), $request->request->get('csrf_token')) ) 
        {
            ...
            else //Dans le cas contraire
            {
                    ...
                    //Générer le message flash correspondant 
                    $this->addFlash('success',"L'article à été retiré de la liste des publications.");

            }
            

            // Demander au Entity manager de préparer la requête de modification du post (persis)
            $em->persist($post);
            // Demander au EM d'executer la requête 
            $em->flush();
        ...
        ) : Response 
    {
        if (count($categoryRepository->findAll()) <=0 ) 
        {
            $this->addFlash('warning', "Vous devez créer au moins une catégorie avant de modifier un article.");
            return $this->redirectToRoute('admin.category.index');
        ...
        if ($form->isSubmitted() && $form->isValid()) 
        {
            ...
            $this->addFlash('success', "Ce article a été modifié.");
